...
namespace BigSchool_THBuoi4.Controllers
{
    ...
    {
        [HttpPost]
        public IHttpActionResult Follow(Followings follow)
        {
            //user login là người theo dõi, follow.FolloweeId là người được theo dõi
            var userID = User.Identity.GetUserId();  
            if (userID == null)
                return BadRequest("Please login first!");
            if (userID == follow.FolloweeId) return BadRequest("Can not follow myself!");
            BigSchoolContext context = new BigSchoolContext();
            //kiểm tra xem mã userID đã được theo dõi chưa 
            Followings find = context.Followings.FirstOrDefault(p => p.FollowerId == userID && p.FolloweeId == follow.FolloweeId);  
            if (find != null)
            {
            ...
            context.Followings.Add(follow); context.SaveChanges();
            return Ok();
        }
    }
